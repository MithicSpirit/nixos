      ...
      lib.mkMerge [
        {
          ...
          xwayland-satellite.path = "${lib.getExe config.wayland.windowManager.niri.xwaylandSatellitePackage}";

          _children =
            toChildren "window-rule" [
              {
            ...
            ++ toChildren "layer-rule" [
              {
                ...
                _children = toChildren "match" (map (x: {_props.namespace = "^${x}$";}) [
                  "waybar"
                  "menu"
                  "notifications"
                ]);
